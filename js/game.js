...
// ===== Elements (safe: may not exist on older HTML) =====
...
// Toggle controls (optional; only if your HTML has them)
...
if (hardLockOnBtn && hardLockOffBtn) {
  hardLockOnBtn.onclick = () => {
    HARD_LOCK = true;
    setPillActive(hardLockOnBtn, hardLockOffBtn, true);
    if (msgEl) msgEl.textContent = "Hard Lock ON.";
    render();
  };
  hardLockOffBtn.onclick = () => {
    HARD_LOCK = false;
    setPillActive(hardLockOnBtn, hardLockOffBtn, false);
    if (msgEl) msgEl.textContent = "Hard Lock OFF (testing mode).";
    render();
  };
}
if (aiModeMenaceBtn && aiModeNormalBtn) {
  aiModeMenaceBtn.onclick = () => {
    AI_MODE = "MENACE";
    setAiModePills(aiModeMenaceBtn, aiModeNormalBtn, AI_MODE);
    if (msgEl) msgEl.textContent = "AI Mode: MENACE.";
    render();
  };
  aiModeNormalBtn.onclick = () => {
    AI_MODE = "NORMAL";
    setAiModePills(aiModeMenaceBtn, aiModeNormalBtn, AI_MODE);
    if (msgEl) msgEl.textContent = "AI Mode: NORMAL.";
    render();
  };
}
...
// Pluck carries from last hand into next deal (per your rule: new deal must occur before pluck)
...
function makeCardFace(cardStr, disabled=false) {
  ...
  img.onerror = () => {
    // replace with fallback element (image missing)
    const fallback = makeCardFaceFallback(cardStr, disabled);
    try { el.replaceWith(fallback); } catch { /* ignore */ }
  };
...
// ===== Sort: Jokers first, then suits (pre-trump), then trump-first after pick =====
...
// Plucker order: most plucks first. Tie-break by dealer, then left, then right (table position tiebreak)
...
// Victim order: most plucks suffered first.
...
// Build queue pairing plucks earned against plucks suffered
...
// Only checks plucker’s own hand + per-pair suit restriction (keeps chaos; no victim hand reading)
...
// Real attempt needs victim high card; if victim has none => pluck LOST (per rules)
...
// Public-only preview (does not inspect victim hand)
...
function illegalReason(playerIndex, cardStr) {
  // Trick 1 lead must be 2C if you have it (forced lead rule)
  ...
  // If leading and trump not open, cannot lead trump if you have a non-trump
...
function legalIndexesFor(playerIndex) {
  ...
  // Forced first lead: 2C if in hand
  if (trickNumber === 1 && trick.length === 0 && hand.includes(CARD_OPEN_LEAD)) {
    return hand.map((c,i)=>({c,i})).filter(x=>x.c === CARD_OPEN_LEAD).map(x=>x.i);
  }

  // Trump not open: if leading, must lead non-trump if you have it
  if (trick.length === 0 && !trumpOpen && trumpSuit !== "C") {
...
function scoreLeadCard_NORMAL(playerIndex, cardStr) {
  // Basic: lead low when safe, avoid burning trump/jokers early
  const v = cardVal(cardStr);
  const trump = isTrumpCard(cardStr, trumpSuit);
  let score = 0;

  score -= v * 4; // prefer lower leads
  if (trump && !trumpOpen) score -= 500;
  if (isJoker(cardStr)) score -= 900;

  // If you still need tricks, slightly prefer higher leads
  const need = players[playerIndex].quota - players[playerIndex].tricks;
  if (need > 0) score += v * 2;

  return score;
}
function scoreFollowCard_NORMAL(playerIndex, cardStr) {
  // Basic: follow suit low unless winning helps you hit quota
  const v = cardVal(cardStr);
  const trump = isTrumpCard(cardStr, trumpSuit);
  const need = players[playerIndex].quota - players[playerIndex].tricks;
  ...
  if (need > 0 && winsNow) score += 400 + v * 2;
  if (need < 0 && winsNow) score -= 600; // avoid extra books

  return score;
}

// MENACE: pushes opponents off quota, uses void memory + punishes
function scoreLeadCard_MENACE(playerIndex, cardStr) {
  const v = cardVal(cardStr);
  const trump = isTrumpCard(cardStr, trumpSuit);
...
function scoreFollowCard_MENACE(playerIndex, cardStr) {
  const v = cardVal(cardStr);
  const trump = isTrumpCard(cardStr, trumpSuit);
  const neededSelf = players[playerIndex].quota - players[playerIndex].tricks;
  const opp = opponentNeeds(playerIndex);
...
function chooseAiIndex(playerIndex) {
  ...
  for (const idx of legal) {
    ...
    if (AI_MODE === "NORMAL") {
      score = leading ? scoreLeadCard_NORMAL(playerIndex, card) : scoreFollowCard_NORMAL(playerIndex, card);
    } else {
      score = leading ? scoreLeadCard_MENACE(playerIndex, card) : scoreFollowCard_MENACE(playerIndex, card);
    }
...
function startPluckPhaseAfterDeal() {
  ...
  // If there were pending plucks from last hand, use them now. Otherwise none.
  pluckQueue = (pendingPluckQueue && pendingPluckQueue.length) ? pendingPluckQueue.slice() : [];
  pendingPluckQueue = null;

  if (pluckQueue.length === 0) {
    if (msgEl) msgEl.textContent = "No plucks this hand. Moving to trump pick.";
    moveToTrumpPick();
  } else {
...
// ===== New Deal entry point =====
function startNewDeal() {
  // rotate dealer FIRST (including first load)
...
if (resetBtn) resetBtn.addEventListener("click", () => startNewDeal());
